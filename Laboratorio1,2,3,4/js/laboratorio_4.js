function generaTabla() {
  ...
  if (!isNaN(numero)) {
    ...
    for (let i = 0; i <= numero; i++) {
      ...
      // y no "" o ''
      tablaHTML += `<tr><td>${i}</td><td>${i * i}</td><td>${
        i * i * i
      }</td></tr>`;
    }
    ...
    //Imprimir tabla
    document
      .getElementById("tablaDeCuadrados")
      .insertAdjacentHTML("afterend", tablaHTML);
  } else {
...
function numerosRGN() {
  ...
  if (parseInt(resUsu) == resCor) {
    ...
    alert(
      "Lo siento, respuesta incorrecta, te tardaste: " + timpoRes + " segundos."
    );
  }
}
...
document
  .getElementById("tablaDeCuadrados")
  .addEventListener("click", generaTabla);
